...
2. **Traffic & liveness** — keep the node able to submit. [Traffic top-up](traffic-topup) buys CIP-0104 synchronizer traffic before the balance runs out, as a dynamic layer over the validator-app's own steady-state loop; the [fleet-watchdog](fleet-watchdog) adds cross-member rescue so one validator can fund another before it stalls, plus a Canton Coin balance alarm.
